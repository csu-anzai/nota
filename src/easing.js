...
export const ease = (current, original, target, changedAt, secs, duration, easingFn, threshold = 1) => {
  if (typeof target !== 'number') return current;

  return Math.abs(target - current) > threshold
    ? easingFn(secs - changedAt, original, target - original, duration)
    : target;
};

export const DURATION_EASING_MS = 1000;
